...
Proves prism.trust can load a service identity, sign a service JWT,
...
def _write_keyset(keys_dir, name="crystal"):
...
def test_sign_and_verify_standalone(tmp_path, monkeypatch):
    ...
    _write_keyset(tmp_path, "crystal")
    ...
    service_identity.init_service_identity("crystal")
    token = service_identity.sign_service_jwt(audience="mantle")

    claims = authority_trust.verify_jwt(
        token, expected_issuer_service="crystal", expected_issuer_claim="crystal")
...
def test_delegation_round_trip(tmp_path, monkeypatch):
    ...
    claims = authority_trust.verify_delegation_jwt(
        token, expected_issuer="crystal", expected_audience="agience-server-x",
        expected_actor="crystal")
    assert claims["sub"] == "user-42"
    assert claims["act"]["sub"] == "crystal"
